...
Working today: every Jolt subsystem, across **1451 C entry points** — shapes,
...
|  | joltc | zjolt |
|---|---|---|
| entry points returning an error code | none | 776 |
...
than the couple of seconds a smaller library would take. `--full` is 29 checks
— 21 of them run on this host, 8 cross-compiling — which is minutes from a
...
Every Jolt subsystem is bound: **1451 C entry points** across **25 headers**,
...
no method reaches. **1360 are spelled out by an entry point** of a matching
name. The other **1322 carry a recorded verdict** in `tools/verdicts_*.txt`,
one line each with its evidence: **604 `BOUND`** (the effect is reachable, by
