...
pub fn build(b: *std.Build) void {
    ...
    // And the one header that is not in the tree: what a consumer's compiler
    // has to agree about, written from the options this library was compiled
    // with. `linkLibrary` propagates include paths and libraries, never -D
    // flags, so a C host had no way to learn them and got
    // ZJOLT_RESULT_CONFIG_MISMATCH at init when it guessed wrong -- or, for
    // ZJOLT_SHARED on the MSVC ABI, an unresolved symbol at link.
    const config_header = b.addWriteFiles().add("zjolt_config.h", b.fmt(
        \\/* Generated by zjolt's build.zig. Do not edit; edit the options.
        \\   Installed beside the public headers and included by zjolt_core.h,
        \\   so a consumer that has these headers needs no -D of its own. */
        \\#ifndef ZJOLT_CONFIG_H_
        \\#define ZJOLT_CONFIG_H_
        \\
        \\{s}#ifndef ZJOLT_OBJECT_LAYER_BITS
        \\#define ZJOLT_OBJECT_LAYER_BITS {d}
        \\#endif
        \\{s}
        \\#endif /* ZJOLT_CONFIG_H_ */
        \\
    , .{
        if (options.double_precision)
            "#ifndef ZJOLT_DOUBLE_PRECISION\n#define ZJOLT_DOUBLE_PRECISION\n#endif\n\n"
        else
            "",
        options.object_layer_bits,
        if (options.shared)
    ...
    lib.installHeader(config_header, "zjolt_config.h");

    // On the library's own include path too, so the in-repo build reads the
    // same file a consumer does rather than a second description of it.
    lib.root_module.addIncludePath(config_header.dirname());
    ...
    c_smoke.root_module.addIncludePath(config_header.dirname());
